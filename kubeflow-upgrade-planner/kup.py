...
def transform(bundle, get_revision=False):
    charm_version_dict = {}
    if not get_revision:
...
def upgrade_flagger(source, target):
    final_dict = {}
    num_changes = 0
...
def get_reversion_numbers(bundle):
...
def download_bundle(target_version):
    target_bundle = None
    version, channel = target_version.split("/")
    url = f"https://github.com/canonical/bundle-kubeflow/raw/main/releases/{version}/{channel}/kubeflow/bundle.yaml"
...
if __name__ == '__main__':
    ...
    if file:
        # get local bundle
        local_bundle = load_bundle(file)
        charm_version_dict, local_version = transform(local_bundle)
        if not target_version:
            pprint(charm_version_dict)
            exit()
    ...
    if not target_version:
        ...
        exit()
    # get target bundle
    target_bundle = download_bundle(target_version)
    if not target_bundle:
        exit()
    charm_version_dict_target, target_version = transform(target_bundle, get_revision=True)
    if not file:
        pprint(charm_version_dict_target)
        exit()

    # print upgrade opportunities
    upgrade_flagger(source=charm_version_dict, target=charm_version_dict_target)
